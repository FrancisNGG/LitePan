...
func defaultSpecs() []Spec {
	return []Spec{
		...
		stringSpec(KeyMOMovieNamingFormat, "media_organize", "电影命名模板", "Jinja2 语法，MoviePilot 兼容，渲染完整路径（目录/文件名）。留空则使用任务级模板。", `{% if title %}{{ title }}{% endif %}{% if en_title %} {{ en_title }}{% endif %}{% if year %} ({{ year }}){% endif %}/{% if title %}{{ title }}{% endif %}{% if en_title %} {{ en_title }}{% endif %}{% if year %} ({{ year }}){% endif %}{% if part %}-{{ part }}{% endif %} - [{% set has_any = false %}{% if webSource %}{{ webSource }}{% set has_any = true %}{% endif %}{% if resourceType %}{% if has_any %} {% endif %}{{ resourceType }}{% set has_any = true %}{% endif %}{% if effect %}{% if has_any %} {% endif %}{{ effect }}{% set has_any = true %}{% endif %}{% if ('remux' in original_name.lower()) %}{% if has_any %} {% endif %}Remux{% set has_any = true %}{% endif %}{% if videoFormat %}{% if has_any %} {% endif %}{{ videoFormat }}{% set has_any = true %}{% endif %}{% if videoCodec %}{% if has_any %} {% endif %}{{ videoCodec }}{% set has_any = true %}{% endif %}{% if audioCodec %}{% if has_any %} {% endif %}{{ audioCodec }}{% set has_any = true %}{% endif %}{% if releaseGroup %}{% if has_any %} {% endif %}{{ releaseGroup }}{% set has_any = true %}{% endif %}]{% if fileExt %}{{ fileExt }}{% endif %}`),
		stringSpec(KeyMOTVNamingFormat, "media_organize", "电视剧命名模板", "Jinja2 语法，MoviePilot 兼容，渲染完整路径（目录/季目录/文件名）。留空则使用任务级模板。", `{% if title %}{{ title }}{% endif %}{% if en_title %} {{ en_title }}{% endif %}{% if year %} ({{year}}){% endif %}/Season {{season}}/{% if title %}{{ title }}{% endif %}{% if en_title %} {{ en_title }}{% endif %} - {{season_episode}}{% if part %}-{{part}}{% endif %}{% if episode %} - [{% set has_any = false %}{% if webSource %}{{ webSource }}{% set has_any = true %}{% endif %}{% if resourceType %}{% if has_any %} {% endif %}{{ resourceType }}{% set has_any = true %}{% endif %}{% if effect %}{% if has_any %} {% endif %}{{ effect }}{% set has_any = true %}{% endif %}{% if ('remux' in original_name.lower()) %}{% if has_any %} {% endif %}Remux{% set has_any = true %}{% endif %}{% if videoFormat %}{% if has_any %} {% endif %}{{ videoFormat }}{% set has_any = true %}{% endif %}{% if videoCodec %}{% if has_any %} {% endif %}{{ videoCodec }}{% set has_any = true %}{% endif %}{% if audioCodec %}{% if has_any %} {% endif %}{{ audioCodec }}{% set has_any = true %}{% endif %}{% if releaseGroup %}{% if has_any %} {% endif %}{{ releaseGroup }}{% set has_any = true %}{% endif %}] - 第 {{episode}} 集{% endif %}{% if fileExt %}{{ fileExt }}{% endif %}`),
		stringSpec(KeyMOCategoryMap, "media_organize", "分类策略", "按 TMDB 元数据自动分类（MoviePilot 一致）。空=不分类。", `{"movie":{"动画电影":{"genre_ids":"16"},"华语电影":{"original_language":"zh,cn,bo,za"},"外语电影":{}},"tv":{"国漫":{"genre_ids":"16","origin_country":"CN,TW,HK"},"日番":{"genre_ids":"16","origin_country":"JP"},"纪录片":{"genre_ids":"99"},"儿童":{"genre_ids":"10762"},"综艺":{"genre_ids":"10764,10767"},"国产剧":{"origin_country":"CN,TW,HK"},"欧美剧":{"origin_country":"US,FR,GB,DE,ES,IT,NL,PT,RU,UK"},"日韩剧":{"origin_country":"JP,KP,KR,TH,IN,SG"},"未分类":{}}}`),
