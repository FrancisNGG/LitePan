...
- **文件名解析**：支持广告前缀剥离、中文方括号标签剥离、编码识别（H.264 / H.265 多写法统一形态）、色深（10bit）、音频编码+声道+特效合并（`DDP 5.1 Atmos`）、流媒体平台识别（Netflix / HBO / Disney+ / iTunes 等 400+ 简称与全称）、视频特效（HDR10+ / DoVi）、制作组内置表 + 自定义输入框
- **整理预览**：模板测试实时查 TMDB 验证结果，支持手动匹配

> [!NOTE]
> 文件名解析增强（中文标签剥离、季目录识别、编码数字误判修正等）属于解析质量改进，**与「媒体整理增强」开关无关，始终生效**；「媒体整理增强」开关只控制命名模板与自动分类。
